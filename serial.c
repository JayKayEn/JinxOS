...
irq_handler_serial(struct regs* r) {
    ...
    if (serial) {
        if (!(inb(SERIO + COM_LSR) & COM_LSR_DATA))
            return;

        char c = (char) inb(SERIO + COM_RX);
...
init_serial(void) {
    // Turn off the FIFO
    outb(SERIO + COM_FCR, 0);

    // Set speed; requires DLAB latch
    outb(SERIO + COM_LCR, COM_LCR_DLAB);
    outb(SERIO + COM_DLL, (uint8_t) (115200 / 9600));
    outb(SERIO + COM_DLM, 0);

    // 8 data bits, 1 stop bit, parity off; turn off DLAB latch
    outb(SERIO + COM_LCR, COM_LCR_WLEN8 & ~COM_LCR_DLAB);

    // No modem controls
    outb(SERIO + COM_MCR, 0);
    // Enable rcv interrupts
    outb(SERIO + COM_IER, COM_IER_RDI);

    // Clear any preexisting overrun indications and interrupts
    // Serial port doesn't exist if COM_LSR returns 0xFF
    serial = (inb(SERIO + COM_LSR) != 0xFF);
    (void) inb(SERIO + COM_IIR);
    (void) inb(SERIO + COM_RX);
